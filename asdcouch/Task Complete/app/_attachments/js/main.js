...
$(document).on('pageshow', '#editpage', function () {
    ...
    if (viewName.task) {
        $.couch.db("tasks").openDoc(viewName.task, {
            success: function (response) {
                // console.log(response);
                var s = new Date(response.startDate),
                    sDate = s.toLocaleDateString(),
                    e = new Date(response.endDate),
                    eDate = e.toLocaleDateString();
...
var storeData = function(data, keys) {
    console.log(keys);
    var id = typeof keys !== 'undefined' ? keys : null;
    ...
    $.couch.db("tasks").removeDoc(doc, {
        success : function() {
            ...
            $('<div>').simpledialog2({
                ...
                zindex: 1000,
                blankContent :
                    "<ul data-role='listview'><li>The task has been deleted successfully!</li>"+
                    "<a rel='close' data-role='button' href='#'>Close</a>",
                callbackClose: refreshPage
            });
        }
